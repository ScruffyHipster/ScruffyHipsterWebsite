...
To block websites on iPhone, use Screen Time content restrictions or an app blocker that supports website blocking. This is important because many distractions move between apps and the browser. Blocking only the app is not enough if the same feed is still one Safari search away.
